      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-3xl mb-16">
          ...
          <h2 className="text-4xl sm:text-6xl font-syne font-black text-white tracking-tighter uppercase leading-none">
            Transparent Matrix<span className="text-[#C8102E]">.</span>
          </h2>
        ...
        <div className="overflow-x-auto mb-20 border border-white/10 bg-[#0a0a0a]">
          <table className="w-full text-left border-collapse min-w-[650px]">
            <thead>
              <tr className="bg-[#C8102E] text-white border-b border-white/20 font-mono text-xs uppercase tracking-[0.2em]">
                ...
                <th className="py-4 px-6 font-bold text-[#ff8093]">
                  Luxury & Custom Scope
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/10 text-xs font-mono">
              {PRICING_TABLE_DATA.map((row, idx) => (
                ...
                >
                  <td className="py-5 px-6 font-syne font-bold text-white group-hover:text-[#ff8093] transition-colors flex items-center space-x-2">
                  ...
                  <td className="py-5 px-6 text-[#ff8093] font-bold bg-[#C8102E]/20">
                    {row.luxuryFull}
                  </td>
                </tr>
              ))}
            ...
            {SPECIALIZATION_SECTORS.map((sector, idx) => (
              <div
                ...
                className={`p-6 border transition-all ${
                  sector.primary
                    ? 'bg-[#C8102E] text-white border-white/20 shadow-2xl relative overflow-hidden'
                    : 'bg-[#0a0a0a] text-white border-white/10 hover:border-white/30'
                }`}
              >
                {sector.primary && (
                  <div className="absolute top-3 right-3 bg-black text-[#ff8093] border border-white/20 px-3 py-0.5 text-[8px] font-mono font-bold uppercase tracking-[0.25em]">
                    CORE FOCUS
                  </div>
                )}

                <div className="flex items-center space-x-3 mb-3">
                  <Building2 size={20} className={sector.primary ? 'text-[#ff8093]' : 'text-[#C8102E]'} />
                  <h4 className="text-lg font-syne font-black uppercase text-white">
                    {sector.title}
                  </h4>
                </div>
